...
import { AlertTriangle, CheckCircle2, Eye, Plus, RotateCcw, Save, Send, Trash2 } from 'lucide-react';
...
function normalizePayload(input: any): DraftPayload {
...
export default function CreatorEditor() {
  ...
  const loadDraft = async (targetComicId: string) => {
    ...
    try {
      ...
      const normalized = normalizePayload(incoming);
          ...
          <section className="creator-editor-canvas-column">
                ...
                <Field label="Жанры (через запятую)">
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      updateMeta({ genres: splitByComma(value) });
                    }}
                    placeholder="фэнтези, хоррор"
                  />
                </Field>
            ...
            <Card className="stack">
              ...
              <div className="notice">
                Рабочая область открыта целиком: кнопки можно ставить на изображение, сбоку или ниже кадра.
                Перетаскивайте кнопку мышью, нижним правым углом меняйте размер.
              </div>

              <div className="editor-workspace-scroll">
                <div
                  ref={workspaceRef}
