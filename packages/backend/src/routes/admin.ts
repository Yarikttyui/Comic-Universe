...
function normalizePayload(payload: StudioPayload) {
  if (payload?.schemaVersion === 2 && payload.comicMeta && Array.isArray(payload.nodes)) {
    return {
      comicMeta: payload.comicMeta,
      nodes: payload.nodes,
    };
  }

  const legacyComic = payload?.comic || {};
  const legacyPages = Array.isArray(payload?.pages) ? payload.pages : [];

  return {
    comicMeta: {
      title: legacyComic.title,
      description: legacyComic.description,
      coverImage: legacyComic.coverImage,
      genres: legacyComic.genres || [],
      tags: legacyComic.tags || [],
      startNodeId: legacyComic.startPageId,
      estimatedMinutes: legacyComic.estimatedMinutes,
    },
    nodes: legacyPages.map((page: any, index: number) => ({
      id: page.pageId || `node-${index + 1}`,
      title: page.title,
      imageFileId: null,
      ...
      buttons: Array.isArray(page.choices)
        ? page.choices.map((choice: any, choiceIndex: number) => ({
            id: choice.id || choice.choiceId || `btn-${choiceIndex + 1}`,
            text: choice.text,
...
router.post('/revisions/:revisionId/approve', async (req: AuthRequest, res, next) => {
  try {
    ...
    const normalized = normalizePayload(revision.payloadJson as StudioPayload);
